...
export class GoalTrackingService {
  ...
  public calculateRequiredGrowthRate(currentValue: number, daysElapsed: number): number {
    ...
    const dailyRate = Math.pow(this.FINAL_GOAL / currentValue, 1 / remainingDays) - 1;
    return dailyRate * 365; // Convert to annual rate
  }

  public getCurrentMilestone(currentValue: number): MilestoneConfig {
    return this.milestones.find(m => m.target > currentValue) || this.milestones[this.milestones.length - 1];
  }
  ...
  public calculateProgress(
    ...
    const currentGrowthRate = this.calculateCurrentGrowthRate(recentDailyReturns);
    const requiredGrowthRate = this.calculateRequiredGrowthRate(currentValue, daysElapsed);
    
    // Project time to goal based on current growth rate
    const projectedDays = this.projectTimeToGoal(currentValue, currentGrowthRate);
  ...
  private calculateCurrentGrowthRate(recentDailyReturns: number[]): number {
    ...
    const avgDailyReturn = recentDailyReturns.reduce((a, b) => a + b, 0) / recentDailyReturns.length;
    
    // Convert to annual rate
    return Math.pow(1 + avgDailyReturn, 365) - 1;
  }

  private projectTimeToGoal(currentValue: number, currentGrowthRate: number): number {
  ...
  public getProgressSummary(metrics: ProgressMetrics): string {
    const { currentValue, percentageToGoal, projectedTimeToGoal, currentGrowthRate, requiredGrowthRate, nextMilestone } = metrics;
...
• Projection: ${projectedTimeToGoal === Infinity ? 
