...
export class BacktestingAgent {
  ...
  public async runBacktest(
    ...
    for (let i = 30; i < historicalData.length; i++) {
      // Start from 30 to have enough data for indicators
      ...
      if (
        ...
        const exitPrice = this.adjustPrice(
          currentPrice,
    ...
    return {
      totalReturns:
        ((currentCapital - this.initialCapital) / this.initialCapital) * 100,
  ...
  private adjustPrice(price: number, action: 'buy' | 'sell'): number {
    const slippageAdjustment =
      price * this.slippage * (action === 'buy' ? 1 : -1);
